...
(function(){
  ...
  function go(){
    ...
    fetch('/submit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name,password:pw})})
      ...
      .catch(function(e){setRes('❌ Network error','#f44');btn.disabled=false;btn.textContent='🚀 ANALYZE PASSWORD';});
  }
  function loadLB(){
...
(function(){
  ...
  function doLogin(){
    ...
    loadData().then(function(ok){
      ...
      else{document.getElementById('LER').textContent='❌ Wrong password';TOKEN='';}
    });
  }
...
initDB().then(() => {
    server.listen(PORT, '0.0.0.0', () => {
        ...
        console.log('\n'+'═'.repeat(52));
        console.log('  🔥 ULTIMATE CTF v6.0  [MySQL Edition]');
